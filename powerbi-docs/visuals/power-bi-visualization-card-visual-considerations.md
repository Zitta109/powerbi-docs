...
The (new) Card visual currently does not support sorting data in ascending or descending order directly. However, there is a workaround: you can create a calculated column that ranks the data used in the Small Multiples field. By adding this ranking column as a secondary field in the Small Multiples field, the "More options" (...) menu now will display the "Sort by" option. This method allows you to implement sorting functionality while using the (new) Card visual.
